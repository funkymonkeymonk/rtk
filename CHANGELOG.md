...
* **jj:** add Jujutsu VCS support with token-optimized output (70-90% reduction)
  - `rtk jj status`: Compact working copy state
  - `rtk jj log`: Truncated entries with stripped emails/timestamps
  - `rtk jj diff`: Stat summary + compact hunks
  - `rtk jj show`: Header + compact diff
  - `rtk jj describe/new/squash/absorb/rebase`: "ok ✓" confirmations
  - `rtk jj bookmark`: Compact list or confirmation
  - `rtk jj git push/fetch`: Compact confirmations
  - `rtk jj op log`: Compact operation history
  - `rtk jj undo`: "ok ✓ undone" confirmation
